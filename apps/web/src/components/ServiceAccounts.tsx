...
import { ServiceAccountsApi } from "../lib/apiClient";
...
export function ServiceAccountsPanel() {
  ...
  const load = async()=>{ if(!projectId) return; setLoading(true); try{setItems(await ServiceAccountsApi.list(projectId))}catch(e:any){addToast(e.message,"error")}finally{setLoading(false)} };
  useEffect(()=>{load()},[projectId]);
  const create = async()=>{ if(!projectId) return; try{await ServiceAccountsApi.create(projectId,form);addToast("Service Account created","success");setShow(false);load()}catch(e:any){addToast(e.message,"error")} };
  const del = async(id:string)=>{ if(!projectId) return; try{await ServiceAccountsApi.delete(projectId,id);addToast("Deleted","success");load()}catch(e:any){addToast(e.message,"error")} };
