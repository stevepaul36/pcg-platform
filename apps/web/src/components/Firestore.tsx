...
import { FirestoreApi, formatApiError } from "../lib/apiClient";
...
export function FirestorePanel() {
  ...
  const load = async()=>{ if(!projectId) return; setLoading(true); try{setItems(await FirestoreApi.list(projectId))}catch(e:any){addToast(formatApiError(e),"error")}finally{setLoading(false)} };
  useEffect(()=>{load()},[projectId]);
  const create = async()=>{ if(!projectId) return; try{await FirestoreApi.create(projectId,form);addToast("Firestore Database created","success");setShow(false);load()}catch(e:any){addToast(formatApiError(e),"error")} };
  const del = async(id:string)=>{ if(!projectId) return; try{await FirestoreApi.delete(projectId,id);addToast("Deleted","success");load()}catch(e:any){addToast(formatApiError(e),"error")} };
  return(<div className="p-6 space-y-6">
    ...
    {show&&<div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50"><div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-md space-y-4">
      ...
      <p className="text-xs text-gcp-muted mt-0.5">Use (default) or lowercase name (e.g. my-db)</p>
