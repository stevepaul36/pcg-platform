...
export function CloudDeployPanelPanel() {
  ...
  const create = async()=>{ if(!projectId) return; try{await DeployApi.create(projectId,form);addToast("Pipeline created","success");setShow(false);load()}catch(e:any){addToast(e.message,"error")} };
  ...
  return(<div className="p-6 space-y-6">
    <div className="flex items-center justify-between"><h1 className="text-xl font-semibold flex items-center gap-2"><Rocket className="w-5 h-5 text-gcp-blue"/>Pipelines</h1>
    ...
    <div className="space-y-3">{items.map((i:any)=><div key={i.id} className="card p-4 flex items-center justify-between">
      <div><div className="font-medium">{i.name||i.displayName}</div><div className="text-xs text-gcp-muted mt-1">{`${i.region} · ${(i.stages||[]).length} stages · ${i.lastDeployStatus}`}</div></div>
