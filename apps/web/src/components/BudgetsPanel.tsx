...
export function BudgetsPanelPanel() {
  ...
  return(<div className="p-6 space-y-6">
    ...
    <div className="space-y-3">{items.map((i:any)=><div key={i.id} className="card p-4 flex items-center justify-between">
      <div><div className="font-medium">{i.name||i.displayName}</div><div className="text-xs text-gcp-muted mt-1">{`$$${i.amountUSD} budget · $$${i.currentSpend?.toFixed(2)} spent`}</div></div>
