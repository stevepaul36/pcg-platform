...
import { CloudArmorApi, formatApiError } from "../lib/apiClient";
...
export function CloudArmorPanel() {
  ...
  const load = async()=>{ if(!projectId) return; setLoading(true); try{setPolicies(await CloudArmorApi.list(projectId))}catch(e:any){addToast(formatApiError(e),"error")}finally{setLoading(false)} };
  useEffect(()=>{load()},[projectId]);
  const create = async()=>{ if(!projectId) return; try{await CloudArmorApi.create(projectId,form);addToast("Policy created","success");setShow(false);load()}catch(e:any){addToast(formatApiError(e),"error")} };
  const del = async(id:string)=>{ if(!projectId) return; try{await CloudArmorApi.delete(projectId,id);addToast("Deleted","success");load()}catch(e:any){addToast(formatApiError(e),"error")} };
