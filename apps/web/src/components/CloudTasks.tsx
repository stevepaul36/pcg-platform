...
import { TasksApi, formatApiError } from "../lib/apiClient";
...
export function CloudTasksPanel() {
  ...
  const load = async()=>{ if(!projectId) return; setLoading(true); try{setItems(await TasksApi.list(projectId))}catch(e:any){addToast(formatApiError(e),"error")}finally{setLoading(false)} };
  useEffect(()=>{load()},[projectId]);
  const create = async()=>{ if(!projectId) return; try{await TasksApi.create(projectId,form);addToast("Task Queue created","success");setShow(false);load()}catch(e:any){addToast(formatApiError(e),"error")} };
  const del = async(id:string)=>{ if(!projectId) return; try{await TasksApi.delete(projectId,id);addToast("Deleted","success");load()}catch(e:any){addToast(formatApiError(e),"error")} };
  const toggle = async(id:string,cur:string)=>{ if(!projectId) return; try{await TasksApi.toggle(projectId,id,cur==="RUNNING"?"PAUSED":"RUNNING");load()}catch(e:any){addToast(formatApiError(e),"error")} };
  return(<div className="p-6 space-y-6">
    ...
    {show&&<div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50"><div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-md space-y-4">
      ...
      <p className="text-xs text-gcp-muted mt-0.5">Lowercase letters, digits, hyphens (e.g. my-queue)</p>
