...
import { CloudBuild as CBApi, formatApiError } from "../lib/apiClient";
...
export function CloudBuildPanel() {
  ...
  const load = async()=>{ if(!projectId) return; setLoading(true); try{setTriggers(await CBApi.list(projectId))}catch(e:any){addToast(formatApiError(e),"error")}finally{setLoading(false)} };
  useEffect(()=>{load()},[projectId]);
  const create = async()=>{ if(!projectId) return; try{await CBApi.create(projectId,form);addToast("Build trigger created","success");setShow(false);load()}catch(e:any){addToast(formatApiError(e),"error")} };
  const del = async(id:string)=>{ if(!projectId) return; try{await CBApi.delete(projectId,id);addToast("Deleted","success");load()}catch(e:any){addToast(formatApiError(e),"error")} };
  return(<div className="p-6 space-y-6">
    ...
    {show&&<div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50"><div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-md space-y-4">
      ...
      <p className="text-xs text-gcp-muted mt-0.5">Lowercase letters, digits, hyphens (e.g. my-trigger)</p>
