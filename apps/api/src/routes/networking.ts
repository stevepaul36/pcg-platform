...
networkingRouter.get("/:projectId/vpcs", requireProjectAccess, async (req, res, next) => {
  try {
    const vpcs = await prisma.vPCNetwork.findMany({ where: { projectId: req.params.projectId }, orderBy: { createdAt: "desc" } });
...
networkingRouter.get("/:projectId/loadbalancers", requireProjectAccess, async (req, res, next) => {
  try {
    const lbs = await prisma.loadBalancer.findMany({ where: { projectId: req.params.projectId }, orderBy: { createdAt: "desc" } });
...
networkingRouter.post("/:projectId/loadbalancers", requireProjectAccess, requireProjectWrite, async (req, res, next) => {
  try {
    ...
    const lb = await prisma.loadBalancer.create({ data: { ...body, projectId: req.params.projectId, ip, status: "ACTIVE", hourlyCost: 0.025 } });
    await logActivity(prisma, req.params.projectId, user.email, { type: "LB_CREATE", description: `Created load balancer ${body.name}`, resourceId: lb.id, severity: "INFO" });
...
networkingRouter.delete("/:projectId/loadbalancers/:lbId", requireProjectAccess, requireProjectWrite, async (req, res, next) => {
  try {
    ...
    await logActivity(prisma, req.params.projectId, user.email, { type: "LB_DELETE", description: `Deleted load balancer ${lb.name}`, resourceId: lb.id, severity: "WARNING" });
    res.json({ success: true });
  } catch (err) { next(err); }
});
...
networkingRouter.get("/:projectId/dns", requireProjectAccess, async (req, res, next) => {
  try {
    const zones = await prisma.cloudDNSZone.findMany({ where: { projectId: req.params.projectId }, orderBy: { createdAt: "desc" } });
...
networkingRouter.post("/:projectId/dns", requireProjectAccess, requireProjectWrite, async (req, res, next) => {
  try {
    ...
    const zone = await prisma.cloudDNSZone.create({ data: { ...body, projectId: req.params.projectId, recordCount: 2 } });
    await logActivity(prisma, req.params.projectId, user.email, { type: "DNS_ZONE_CREATE", description: `Created DNS zone ${body.dnsName}`, resourceId: zone.id, severity: "INFO" });
...
networkingRouter.delete("/:projectId/dns/:zoneId", requireProjectAccess, requireProjectWrite, async (req, res, next) => {
  try {
    ...
    await logActivity(prisma, req.params.projectId, user.email, { type: "DNS_ZONE_DELETE", description: `Deleted DNS zone ${zone.dnsName}`, resourceId: zone.id, severity: "WARNING" });
    res.json({ success: true });
  } catch (err) { next(err); }
});
