...
import { Cloud, LogIn, UserPlus, ArrowLeft, Mail } from "lucide-react";
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gcp-muted text-sm mt-1">Cloud Gaming Simulator</p>
        </div>
...
function LoginForm({ onSwitch }: { onSwitch: (v: AuthView) => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {error && <div className="bg-red-50 text-gcp-red text-sm p-3 rounded-md">{error}</div>}
      <div>
      ...
      <div className="flex justify-between text-sm">
        <button type="button" onClick={() => onSwitch("forgot")} className="text-gcp-blue hover:underline">Forgot password?</button>
        <button type="button" onClick={() => onSwitch("register")} className="text-gcp-blue hover:underline">Create account</button>
      </div>
    </form>
  );
}
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <input type="text" className="input-field" value={name} onChange={(e) => setName(e.target.value)} placeholder="Your name" required minLength={2} />
      </div>
      <div>
        <label className="block text-sm font-medium mb-1">Email</label>
        <input type="email" className="input-field" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" required />
      ...
      <div>
        <label className="block text-sm font-medium mb-1">Password</label>
        <input type="password" className="input-field" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Min 8 chars, 1 uppercase, 1 number" required minLength={8} />
      </div>
      <button type="submit" className="btn-primary w-full" disabled={loading}>
        {loading ? "Creating account..." : "Create Account"}
      </button>
      <button type="button" onClick={() => onSwitch("login")} className="text-sm text-gcp-blue hover:underline flex items-center gap-1">
  ...
  if (sent) {
    return (
      <div className="text-center space-y-4">
        ...
        <p className="text-sm text-gcp-muted">If an account with that email exists, we sent a password reset link.</p>
        <button type="button" onClick={() => onSwitch("login")} className="btn-primary">Back to sign in</button>
      </div>
    );
  }
